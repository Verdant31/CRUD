...
export function EnquetesEA() {
  const [enquetesIniciadas, setEnquetesIniciadas] = useState<Enquete[]>();
  ...
  function handleFinishSurvey(id: number) {
  ...
  useEffect(() => {
    axios.get('http://localhost:3001/api/getenquetesi')
      .then((response) => {
        setEnquetesIniciadas(response.data.map((enquete: Enquete) => {
          return {
            id: enquete.id,
            nome: enquete.nome,
  ...
  return (
    <Box align="center">
      <Text mb="8" fontWeight="bold" fontSize="3xl" color="red.100">Enquetes <Text as="span" color="gray.100">{"<"}em andamento{">"}</Text></Text>
      <Box h="450px" w="320px" borderRadius="3xl" borderColor="red.100" borderWidth="4px">
        <Stack direction="column" spacing="3.5" ml="6" mt="4" align="start" borderBottomWidth="2px" borderBottomColor="gray.150" mr="6" >
          {enquetesIniciadas?.map((enquete: Enquete) => {
            return (
              <Box key={enquete.nome}>
                ...
                <Stack mt="2" direction="row" align="center">
                  <Button size="md" width="100px" colorScheme="red" onClick={() => handleFinishSurvey(enquete.id)}>Finalizar</Button>
                  <Button size="md" colorScheme="red" onClick={() => { handleDeleteSurvey(enquete.id) }}><DeleteIcon h="6" w="6" /></Button>
                  <Button size="md" colorScheme="red" >Votar</Button>
